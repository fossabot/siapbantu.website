const VolunteerGroups = {
  ...
  addVolunteerToGroup(that) {
    ...
    const currentAcceptedUserIds = $("#accepted_user_ids")
      .val()
      .split(",")
      .filter((el) => el);
    ...
    $.post(
      `/admin/volunteer_groups/generate_volunteers?project_id=${projectId}`,
      { user_ids: currentAcceptedUserIds, filter },
      (data) => {
...
<li class="border-t border-gray-200 volunteer-in-group">
  <input type="hidden" value="${
    user.id
  }" name="volunteer_group[user_ids][]" class="volunteer-group-user-id" />
  <div class="block focus:outline-none transition duration-150 ease-in-out">
    <div class="px-4 py-4 sm:px-6">
      <div class="flex items-center justify-between">
        <div class="text-sm leading-5 font-medium text-indigo-600 truncate">
          <span class="volunteer-name">${
            user.name
          }</span> / <span class="volunteer-email">${user.email}</span>
        </div>
      ...
      <div class="mt-2 sm:flex sm:justify-between">
        <div class="text-sm leading-5 text-gray-500">
          location: ${user.location} / skills: ${
            user.skill_list ? user.skill_list.join(", ") : ""
          }
        </div>
      </div>
    </div>
...
This project needs your help: ${projectName} (https://siapbantu.com/projects/${projectId}) by project owner - ${projectOwnerEmail}. I thought you might be able to help. I have included the project owner to this email should have any questions.

If you are already on an existing project or feel that you are not the right fit after speaking with the project owner. Please login to siapbantu.com and remove the setting "pair me with a project" from My Profile page.
...
`;
    ...
    const mailHref = `mailto:${projectOwnerEmail}?reply-to=${projectOwnerEmail}&bcc=${volunteerEmails
      .join(",")
      .replace(/\+/g, "%2B")}&subject=${subject}&body=${body
      .replace(/\n/g, "%0D%0A")
      .replace(/\+/g, "%2B")}`;
